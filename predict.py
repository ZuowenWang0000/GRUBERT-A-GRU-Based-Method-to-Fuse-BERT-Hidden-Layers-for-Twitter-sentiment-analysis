...
def main_cli(config, checkpoint, predict_file, embedding):
    ...
    if embedding == "flair":
        ...
        embedding = StackedEmbeddings(
            embeddings=[glove_embedding, syngcn_embedding, flair_forward_embedding, flair_backward_embedding])

        import flair
        from flair.datasets import CSVClassificationDataset
        print("[flair] initializing datasets", flush=True)
        eval_dataset = CSVClassificationDataset(os.path.join(dataset_path, test_file_path), {1: "text", 2: "label"},
                                                max_tokens_per_doc=sentence_length_cut, tokenizer=False,
                                                in_memory=False, skip_header=True)
        ...
        syngcn_embedding = SynGcnEmbedding(dataset_path, train_file_path, test_file_path, sentence_length_cut,
                                           "../embeddings/syngcn_embeddings.txt")

        # dataloader
        eval_loader = torch.utils.data.DataLoader(
            TweetsDataset(glove_embedding.get_test_set(), syngcn_embedding.get_test_set()),
        ...
        eval_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, num_workers=workers,
                                                  shuffle=False)
        embedder = None
